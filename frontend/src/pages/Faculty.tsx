...
export default function Faculty() {
  ...
  useEffect(() => {
    fetch("http://localhost:3001/api/users?role=faculty")
      .then(r => r.json())
      .then(setFaculty);
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100">
      ...
      <div className="max-w-7xl mx-auto p-6 flex">
        ...
        <div className="flex-1 ml-6">
          <h2 className="text-3xl font-bold mb-6">Faculty Management</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-2">Fingerprint ID</th>
                  <th className="text-left py-2">Clearance</th>
              ...
              <tbody>
                {faculty.map(f => (
                  <tr key={f.id} className="border-b">
                    ...
                    <td className="py-2">{f.cardId}</td>
                    <td className="py-2">{f.fingerprintId}</td>
                    <td className="py-2">{f.clearance ? "✅ Cleared" : "❌ Pending"}</td>
                    <td className="py-2">{f.active ? "Active" : "Inactive"}</td>
                    <td className="py-2">
                      <button className="bg-blue-500 text-white px-3 py-1 rounded mr-2">View Logs</button>
                      <button className="bg-green-500 text-white px-3 py-1 rounded mr-2">Clear</button>
                      <button className="bg-red-500 text-white px-3 py-1 rounded">Deactivate</button>
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <div className="w-64 bg-white rounded-2xl shadow-lg p-6">
      <nav className="space-y-4">
        ...
        <button onClick={() => navigate("/faculty")} className="w-full text-left py-2 px-4 rounded-lg bg-blue-100">👨‍🏫 Faculty</button>
